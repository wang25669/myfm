...
这是一个专为**老旧 Android 车机 (Android 4.x)** 量身打造的往意云音乐播放解决方案。它不仅完美解决了老款设备因底层协议过时（不支持 TLS 1.2+）而导致的 `-1004` 等各种网络握手报错，还能在您听歌的同时，自动在您的 NAS/服务器上构建一个永久的**私有离线无损曲库**。
...
> **提示**：
> 运行成功后，会在同目录下生成 `cookies.json`（保存您的登录状态）和 `songs` 文件夹（保存您听过的所有高音质音乐）。
