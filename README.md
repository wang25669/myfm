...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `MYFM_ENDPOINT_UPDATE_URL` | `https://raw.githubusercontent.com/NeteaseCloudMusicApiEnhanced/api-enhanced/main/module/` | 接口配置更新源，默认从 api-enhanced 的模块文件提取接口路径 |
...
默认更新逻辑会读取 `api-enhanced` 中 MyFM 实际用到的模块文件，例如 `song_url_v1.js`、`song_url.js`、`recommend_songs.js`、`personal_fm.js`、`playlist_detail.js`，从其中提取最新的 `url` 路径并写入 `/app/myfm/api_endpoints.json`。如果你将 `MYFM_ENDPOINT_UPDATE_URL` 改成 `.json` 结尾的地址，则会按完整 MyFM 配置 JSON 读取。
